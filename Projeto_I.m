...
%% 12) Especificar as Matrizes associadas ao controle 'B_u', 'D_y', 'D_z'.
...
%% 13) Configuracoes no yalmip

    %13.1) Configuracoes para otimizacao semidefinida no yalmip
    
    Configuracoes_SDP =...
         sdpsettings('verbose',1,'solver','lmilab','debug',1);

    %13.2) Pesos da funcao custo de otimizacao 
    
    c_H2=1;
    c_Hinf=2;

%% 14) Computar quantidade de estados nao alocaveis e checar controlabilidade total
    
    Computar_quantidade_de_estados_nao_alocaveis

%% 15)  Criar matrizes e variaveis de decisao para LMIs 
...
%% 16) Computar as LMIs para controle otimo misto D-estavel

LMIs_controle_misto_D_estavel

%% 19) Executar Algoritmo 3
...
%% 20) Extrair variaveis otimas do Algoritmo 3

optimal_W = value(W);
optimal_X = value(X);

%% 21) Computar Matriz de retroalimentacao via Algoritmo 3

Kpf_classico=optimal_W/optimal_X;

%% 22) Representacao em espaco de estados do sistema em malha fechada 
%   projetado via Algoritmo 3

    % 22.1) Espaco de Estados para Gcdy(s) 
    
    Gcdys_classico=...
        ss(A_x+B_u*Kpf_classico,B_d,C_y+D_y*Kpf_classico,E_y);

    % 22.2) Espaco de Estados para Gcdz(s)  
...
%% 25) Computar Normas H_2 e H_infinito para sistema projetado via 
%   Algoritmo 3

    %25.1) Norma H_2 do sistema canonico controlavel de Gcdy(s)
    
    norma_H2_Gcdys_classico=norm(Gcdys_classico,2);
    
    %25.2) Norma H_infinito do sistema canonico controlavel de Gcdz(s)
    
    norm_Hinf_Gcdzs_classico=norm(Gcdzs_classico,'inf');
    
%% 26) Computar Os polos do sistema projetado via Algoritmo 3
...
%% 28) Executar Algoritmo 7 multietapa

    %28.1) Iniciar matriz de retroalimentacao
    ...
    %28.3) Iniciar contagem de tempo para otimizacao multietapa via 
    %       Algoritmo 7
    tic
    
    %28.4) Laco para realizar otimizacao multietapa
    for j = 1:n_pcc % contagem de par de polos selecionado
        
        %28.4.1) Obter Matrizes dos Autovalores e Autovetores
        
        [Right_Eigenvectors, Eigenvalues, Left_Eigenvectors] = ...
            eig(A_x + B_u * Kpf_parcial);

        %28.4.2) Escolher o j-enesimo par de polos contidos em "Open_Loop_Poles"   
        
        pole1 = Polos_Malha_Aberta(2 * j - 1, 1);
        pole2 = Polos_Malha_Aberta(2 * j, 1);

        %28.4.3) Computar as diferencas entre os autovalores e os polos
        
        dif_pole1 = abs(Eigenvalues - pole1);
        dif_pole2 = abs(Eigenvalues - pole2);

        %28.4.4) Encontrar o indice dos elementos que tem a menor diferenca
        % entre os autovalores e os polos selecionados para alocacao
        
        [~, minIndex_1] = min(dif_pole1(:));
        [~, minIndex_2] = min(dif_pole2(:));

        %28.4.5) Converte o indices lineares "minIndex_1" e "minIndex_2"  
        ...
        %28.4.6) Ordenar os intervalos de linhas e colunas contidos em 
        % "row_poles" e "col_poles".   
        
        row_poles = min(row_1, row_2):max(row_1, row_2);
        col_poles = min(col_1, col_2):max(col_1, col_2);

        %28.4.7) Encontrar a submatrices "Lambda_p" e "L_p"
        ...
        Controle_otimo_misto_D_estavel_alocacao_parcial
            ...
            %28.4.10.11) Executar Algoritmo 7 Multietapa   
            ...
            %28.4.10.12) Extrair variaveis de decisao para computar K_pf 
            
            optimal_Tilde_W = value(Tilde_W);
            optimal_Tilde_X = value(Tilde_X);

            %28.4.10.13) Computar e Atualizar K_pf
    ...
    % 28.5) Finalizar tempo de otimizacao multietapa
    
    tempo_otimizacao_parcial = toc;
    
%% 29) Representacao em espaco de estados do sistema em malha fechada 
%   projetado via Algoritmo 7 multietapa

    % 29.1) Espaco de estados para Gcdy(s) via algoritmo 7 multietapa
    
    Gcdys_parcial=...
        ss(A_x+B_u*Kpf_parcial,B_d,C_y+D_y*Kpf_parcial,E_y);

    % 29.2) Espaco de estados para Gcdz(s) via algoritmo 7 multietapa
    
    Gcdzs_parcial=...
        ss(A_x+B_u*Kpf_parcial,B_d,C_z+D_z*Kpf_parcial,E_z); 

%% 30) Computar Normas H_2 e H_infinito para sistema projetado via Algoritmo 7

    %30.1) Norma H_2 do sistema canonico controlavel de Gcdy(s)
    
    norma_H2_Gcdys_parcial=norm(Gcdys_parcial,2);
    
    %30.2) Norma H_infinito do sistema canonico controlavel de Gcdz(s)
    
    norma_Hinf_Gcdzs_parcial=norm(Gcdzs_parcial,'inf');
    
%% 31) Computar Os polos do sistema via teorema 7

    Polos_Gcd_Parcial=cplxpair(pole(Gcdys_parcial));
    
%% 32) Obter Coeficientes de Amortecimento, Frequencia Natural e Medidas 
...
%% 34) Computar respostas de Gcdy(s) ao disturbio impulso unitario
...
%% 38) Construir graficos de Bode para sistema LCTI-MIMO em malha aberta e 
%   fechada.
...
%% 39) Construir graficos de resposta transitoria de g(t)^2 ao disturbio  
...
%% 40) Construir grafico de u(t) sistema LCTI-MIMO em malha aberta e 
...
%% 41) Grafico para Localizacao dos Polos

    %41.1) Autovalores (Polos) a serem inseridos no grafico
    autovalores=[ Polos_Malha_Aberta',...
                  Polos_Gcd_classico',...  
                  Polos_Gcd_Parcial'];

    %41.2) Computando curvas de fronteira das regioes    
